...
train_input_variables = scaler.fit_transform(train_input_variables)
joblib.dump(scaler, 'scalers/Scaler(CTM__' + version + '__tc60__input).save')

train_prediction_variable = scaler.fit_transform(train_prediction_variable)
...
x = torch.tensor(train_input_variables, dtype = torch.float)
y = torch.tensor(train_prediction_variable, dtype = torch.float)

# Convert Test data to tensors
x_val = torch.tensor(test_input_variables, dtype = torch.float)
y_val = torch.tensor(test_prediction_variable, dtype = torch.float)

### Set hyperparameter
net_input, net_output = x.shape[1], y.shape[1] # input and output layer
h1, h2 = 400, 200 # hidden layers
batchsize = 2048
epochs = 15
learning_rate = 3e-4

# Create RTPT object 
#rtpt = RTPT(name_initials = "TR", experiment_name = "CTM_Net", max_iterations = epochs)
#rtpt.start()
...
# select model 
model = CTM_Net.CTM_Net(net_input, h1, h2, net_output)
...
#plt.yscale('log')
plt.legend()
evaluation_file = "Loss_" + model_name + "_v" + version + "_tc" + str(tc) + "_epochs" + str(epochs) + ".png"
...
#plt.yscale('log')
plt.legend()
evaluation_file = "R2_" + model_name + "_v" + version + "_tc" + str(tc) + "_epochs" + str(epochs) + ".png"
plt.savefig("evaluations/v" + version + "/" + evaluation_file, dpi=1200)